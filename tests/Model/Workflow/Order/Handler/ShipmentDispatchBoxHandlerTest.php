...
use Boodmo\Sales\Model\Workflow\Order\Handler\ShipmentDispatchBoxHandler;
...
class ShipmentDispatchBoxHandlerTest extends TestCase
{
    /**
    ...
    /**
     * @var ShipmentDispatchBoxCommand
     */
    private $command;
    ...
    /**
     * @var ShipmentDispatchBoxHandler
     */
    private $handler;
    ...
    public function setUp()
    {
        ...
        $this->command = new ShipmentDispatchBoxCommand('9806b405-86cd-47c0-8b61-b9d6965935fd', $this->user);
        $this->inputItemList = $this->createMock(InputItemList::class);

        $this->handler = new ShipmentDispatchBoxHandler(
            $this->entityManager,
    ...
    public function testInvoke()
    {
        ...
        $this->shippingService->expects($this->at(1))
            ->method('saveShippingBox')
            ->with($shippingBox);

        ($this->handler)($this->command, $this->orderService);
        $this->assertInstanceOf(\DateTime::class, $shippingBox->getDispatchedAt());
    }
}
